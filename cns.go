...
func handleConn(cConn *net.TCPConn) {
	...
	var rlen, err = cConn.Read(payload)
	if err == nil {
		...
		if isHttpHeader(payload) {
			...
			if enable_httpDNS {
				var domain, err = GetHttpdnsDomain(payload)
				if err == nil {
					RespondHttpdns(cConn, domain)
				}
			} else {
				// process TCP request next
				var WLen, err = cConn.Write(rspHeader(payload))
				if err == nil && WLen > 0 {
...
func initConfig() {
	...
	var configMap = InitConfig(configFile)
